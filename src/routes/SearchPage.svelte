...
<script>
  ...
  function applyFilters(event) {
    ...
    filteredListings = $listings.filter(listing => {
      ...
      if (filters.price.active) {
        if (listing.price < filters.price.min || listing.price > filters.price.max) {
          return false;
        }
      }
      
      // Beds filter
      if (filters.beds.active) {
        if (listing.beds < filters.beds.min || listing.beds > filters.beds.max) {
          return false;
        }
      }
      
      // Baths filter
      if (filters.baths.active) {
        if (listing.baths < filters.baths.min || listing.baths > filters.baths.max) {
          return false;
        }
      }
      
      // Square footage filter
      if (filters.sqft.active) {
        if (listing.sqft < filters.sqft.min || listing.sqft > filters.sqft.max) {
          return false;
        }
      }
      
      // For now, we'll skip amenities and location filtering as they would require additional data
  ...
  onMount(async () => {
    // Initialize map (assuming Leaflet is being used based on the code fragments)
    if (window.L) {
  ...
  // Update map markers when filtered listings change
  $: if (map && filteredListings) {
    updateMapMarkers();
  }
      ...
      if (listing.lat && listing.lon) {
        const marker = L.marker([listing.lat, listing.lon])
          ...
          .bindPopup(`
            ...
            ${listing.beds} bed, ${listing.baths} bath<br>
            ${listing.sqft} sqft
          `);
        markers.push(marker);
      }
...
<div class="search-page">
  ...
  {#if showSidebar}
    <div class="sidebar-container">
      <FiltersSidebar on:filter={applyFilters} />
    </div>
  {/if}
  
  <!-- Listing results (visible when not viewing map) -->
    ...
    {#each filteredListings as listing}
      <div class="listing-card">
        ...
        <div class="listing-info">
          ...
          <p class="details">{listing.beds} bed • {listing.baths} bath • {listing.sqft} sqft</p>
          <button class="favorite-btn" on:click={() => toggleFavorite(listing)}>
            ♥
          </button>
        </div>
      </div>
...
<style>
  ...
  .listing-results {
    display: none; /* Hidden by default when map is shown */
    flex-direction: column;
    gap: 20px;
    padding: 20px;
  }
